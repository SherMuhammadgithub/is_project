...
function Checkout() {
  ...
  const saveBookingToDatabase = async (orderId) => {
    try {
      ...
      showMessageModal(
        "Offer Booked Successfully. Redirecting you to Booked Offers."
      );
      setBooking([]);
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      const apiUrl = "http://localhost:5000/api/orders/create-order";
      ...
      if (!response.ok) {
        if (response.status === 422) {
          showMessageModal(
            "The selected offer is no longer available. Please choose another flight."
          );
        } else {
